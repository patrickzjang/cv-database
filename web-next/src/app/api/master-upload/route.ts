...
/**
 * Apply updates via bulk upsert instead of one UPDATE per row.
 *
 * Previously this sent one DB round-trip per changed row (N+1 problem).
 * Now we merge the patch columns into each row's full data and upsert
 * in chunks of 500 — dramatically faster for large files.
 *
 * We always include ITEM_SKU (the conflict key) plus every key present
 * in the patches so Supabase knows which columns to update on conflict.
 */
async function applyUpdatesFast(
  ...
  // Collect all patch keys so we can build consistent upsert rows.
  const allPatchKeys = Array.from(
    new Set(updates.flatMap((u) => Object.keys(u.patch)))
  );

  const rows = updates.map(({ itemSku, patch }) => {
    const row: Record<string, unknown> = { ITEM_SKU: itemSku };
    for (const k of allPatchKeys) {
      row[k] = patch[k] !== undefined ? patch[k] : null;
    }
    return row;
  });
  ...
  for (const batch of chunk(rows, 500)) {
    const { error } = await supabase
      .schema("public")
      .from(viewTable)
      .upsert(batch, { onConflict: "ITEM_SKU", ignoreDuplicates: false });
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!await checkRateLimit(`master-upload:${ip}`, 10, 60_000)) {
